...
// ... rest of the sections remain the same until AmenitiesSection ...
...
const AmenitiesSection = ({ amenities }: { amenities: ProjectAmenities[] }) => {
  ...
  const x = useTransform(sectionProgress, [0, 0.7], [0, -totalScrollDistance]);
  
  // Sticky header: stick at 20% from top, release after 70% scroll
  const headerY = useTransform(sectionProgress, [0, 0.15, 0.7, 1], [0, 0, 0, 0]);
  const headerOpacity = useTransform(sectionProgress, [0.85, 1], [1, 0]);
  const headerPosition = useTransform(sectionProgress, [0, 0.15, 0.7, 1], ["relative", "fixed", "fixed", "relative"]);

  return (
    <section ref={sectionRef} className="relative bg-champagne-beige/5">
      ...
      <motion.div 
        ...
        style={{ 
          ...
          position: headerPosition as any
        }}
        className="top-[20%] z-40 w-full"
      >
        <div className="container mx-auto px-4 md:px-8 py-12 md:py-16 border-b border-primary/20">
